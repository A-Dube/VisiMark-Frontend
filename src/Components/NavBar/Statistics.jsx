  ...
  const fetchAttendanceData = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setTotalUserAttendance(data.totalUserAttendance || 0);
      } else {
          ...
          <div className="flex justify-between items-end h-48 px-4 overflow-x-auto scrollbar-thin scrollbar-thumb-[#1c3c3c] scrollbar-track-transparent">
            {attendanceData.length > 0 ? (
              attendanceData.map((d, i) => {
                const classHeight =
                  totalClasses > 0
                    ? (d.classAttendance / totalClasses) * 150
                    : 0;
                const userHeight =
                  totalClasses > 0
                    ? (d.userAttendance / totalClasses) * 150
                    : 0;
                return (
              ...
              <p className="text-center text-white/80 w-full">
                No data available
              </p>
            )}
          </div>
